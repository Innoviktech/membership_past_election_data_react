...
import { getConstituancyListByDistrict, getDistrictListByState, getPolingBooth, getPolingStationEditablePdf, getStateList } from '../Store/Middleware/VoterApiServices';
...
const VoterServices = () => {
  ...
  const searchElectoralRoll = async (constituencyId) => {
    ...
    setIsLoading(true);
    SpinnerService.show();
    dispatch(getPolingBooth({constituency_id: constituencyId }))
    .then((res) => {
  ...
  const onHandlePayment = async (e, pdfId) => {
    e.preventDefault();
    setAmount(2);
      try {
  ...
  const convertToEditablePdf = async (pdfId) => {
    // Show loading modal
    ...
    try {
      ...
      if (response?.payload?.file_url) {
        ...
        window.open(editablePdf, '_blank', 'noopener,noreferrer');
      }
    } catch (err) {
      console.log(err);
    ...
    if (Array.isArray(pdfData)) {
      return pdfData.map(item => ({
        ...
        pdfs: (item.pdfs || []).map(pdf => ({
          ...
          file_url: null    // This will be added later from second API
        }))
      }));
    }
    ...
    <>
      ...
      <div className="election-portal">
        ...
        <div className="main-container">
          ...
          <aside
            ref={sidebarRef}
            className={`sidebar ${isMobile ? 'mobile' : 'desktop'} ${isSidebarOpen ? 'open' : ''}`}
          >
            {/* Arrow Toggle Button (only for mobile) */}
            {isMobile && (
              <button
            ...
            <nav>
              <ul className="sidebar-nav">
                <li className={`sidebar-nav-item ${location.pathname === '/voter-services' ? 'active' : ''}`}>
                  <Link
                    to="/voter-services"
                ...
                <li className={`sidebar-nav-item ${location.pathname === '/faqs' ? 'active' : ''}`}>
                  <Link
                    to="/faqs"
                    className="sidebar-nav-link"
                    onClick={isMobile ? closeSidebar : undefined}
                ...
                <li className={`sidebar-nav-item ${location.pathname === '/contact' ? 'active' : ''}`}>
                  <Link
                    to="/contact"
                    className="sidebar-nav-link"
                    onClick={isMobile ? closeSidebar : undefined}
            ...
            <div className="content-wrapper">
              ...
              <div className="search-card">
                ...
                <button
                  ...
                  onClick={() => searchElectoralRoll(selectedConstituency)}
                                  ...
                                  pdf.file_url ? (
                                    ...
                                    <div key={`action-${pdfIndex}`} className="split-cell-row">
                                      <button
                                        ...
                                        onClick={() =>convertToEditablePdf(pdf.attach_id)}
                                        // onClick={(e) => onHandlePayment(e, pdf.attach_id)}
