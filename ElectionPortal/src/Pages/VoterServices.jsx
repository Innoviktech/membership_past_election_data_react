...
import { ChevronDown, Search , Download, FileText, X, Upload } from 'lucide-react';
import './VoterServices.scss';
import { useDispatch } from 'react-redux';
import { convertUploadedPdf, getConstituancyListByDistrict, getDistrictListByState, getPdfPageCount, getPolingBooth, getPolingStationEditablePdf, getPollingBoothListByConstituancy, getStateList, storePaymentDetails } from '../Store/Middleware/VoterApiServices';
...
const VoterServices = () => {
  ...
  const [pollingBoothSearchQuery, setPollingBoothSearchQuery] = useState('');
  ...
  const filteredConstituencies = constituencies.filter(constituency => 
    constituency.constituancy_name.toLowerCase().includes(constituencySearchQuery.toLowerCase())
  );
  
  const filteredPollingBooths = pollingBooths.filter(booth => 
    booth.polling_station_name.toLowerCase().includes(pollingBoothSearchQuery.toLowerCase())
  );
  ...
  if (!selectedState || !selectedDistrict || !selectedConstituency) {
    // alert(t('HOME.SearchElectoralRollAlert'));
    showMessage("alert", t('HOME.SearchElectoralRollAlert'));
    return;
  }
      ...
      try {
        ...
        let options = {
          ...
          handler: function (response) {
            handleStorePaymentDetails(response.razorpay_payment_id);
  ...
  const handleStorePaymentDetails = async (paymentId) => {
    dispatch(storePaymentDetails({payment_id: paymentId}))
    .then((res) => {
      console.log(res);
    })
    .catch((err) => {
  ...
  const convertToEditablePdf = async (pdfId, pdfName) => {
    // Show loading modal
    setModalMessage(t('HOME.LoadingModalMessage'));
    setIsModalOpen(true);
    
    // Set a timeout to show the modal if the API takes too long
    const timeoutId = setTimeout(() => {
      setModalMessage(t('HOME.LoadingModalLongerMessage'));
    ...
    try {
      ...
      setModalMessage(t('HOME.LoadingModalMessageError'));
  ...
  const handleDropdownToggle = (dropdownType) => {
    ...
    // Reset search queries when opening dropdowns
    if (dropdownType === 'state' && !isStateDropdownOpen) setStateSearchQuery('');
    if (dropdownType === 'district' && !isDistrictDropdownOpen) setDistrictSearchQuery('');
    if (dropdownType === 'constituency' && !isConstituencyDropdownOpen) setConstituencySearchQuery('');
    if (dropdownType === 'polling_booth' && !isPollingBoothDropdownOpen) setPollingBoothSearchQuery('');
  };
  ...
  const getSelectedStateName = () => {
    ...
    return state ? state.state_name : t('HOME.SelectState');
  };

  const getSelectedDistrictName = () => {
    const district = districts.find(d => d.district_id === selectedDistrict);
    return district ? district.district_name : t('HOME.SelectDistrict');
  };
  ...
  const getSelectedConstituencyName = () => {
    ...
    if (!constituency) {
      return t('HOME.SelectConstituency');
    }
    
    // Find the district name from your districts array
    const district = districts.find(d => d.district_id === selectedDistrict);
    const districtName = district ? district.district_name : '';
    
    return `${constituency.constituancy_name} - ${districtName}`;
  };

  const getSelectedPollingBoothName = () => {
    const booth = pollingBooths.find(p => p.polling_id === selectedPollingBooth);
    return booth ? booth.polling_station_name : t('HOME.SelectPollingBooth');
  };
  ...
  const downloadImgPDF = (attachId, pdfName, pdfUrl) => {
    const imgPdfUrl = `/api/polling_station_pdf_document?attach_id=${attachId}`;
    const origin = import.meta.env.VITE_ORIGIN === "diff";
    if (origin) {
      downloadPdfWithFileSaver(imgPdfUrl, pdfName);
    }else{
      downloadPdfWithFileSaver(pdfUrl, pdfName);
    }
  }

  // Reusable dropdown search input component
  const DropdownSearch = ({ query, setQuery, placeholder }) => (
            ...
            {activeTab === 'search' ? (
              ...
              <div>
                <div className="search-card">
                  <h1 className="search-title">{t('HOME.SearchElectoralRoll')}</h1>

                  {/* State Selection */}
                  <div className="form-group">
                    <label htmlFor="state-select" className="form-label">
                      {t('HOME.SelectState')}<span className="text-danger"> *</span>
                    </label>
                      ...
                      {isStateDropdownOpen && (
                        <div className="dropdown-menu" role="listbox">
                          <DropdownSearch 
                            ...
                            setQuery={setStateSearchQuery}
                            placeholder={t('HOME.SearchStates')}
                          />
                          <div className="dropdown-list">
                            {filteredStates.length > 0 ? (
                              ...
                              <div className="no-results">{t('HOME.NoStatesFound')}</div>
                            )}
                          </div>
                        </div>
                      )}
                  ...
                  <div className="form-group">
                    <label htmlFor="district-select" className="form-label">
                      {t('HOME.SelectDistrict')}<span className="text-danger"> *</span>
                    </label>
                    <div className="dropdown-container">
                      <button
                        className="dropdown-button"
                      ...
                      {isDistrictDropdownOpen && districts.length > 0 && (
                        <div className="dropdown-menu" role="listbox">
                          <DropdownSearch 
                            ...
                            setQuery={setDistrictSearchQuery}
                            placeholder={t('HOME.SearchDistricts')}
                          />
                          <div className="dropdown-list">
                            {filteredDistricts.length > 0 ? (
                              ...
                              <div className="no-results">{t('HOME.NoDistrictsFound')}</div>
                            )}
                          </div>
                        </div>
                      )}
                  ...
                  <div className="form-group">
                    <label htmlFor="constituency-select" className="form-label">
                      {t('HOME.SelectConstituency')}<span className="text-danger"> *</span>
                    </label>
                    <div className="dropdown-container">
                      <button
                        className="dropdown-button"
                      ...
                      {isConstituencyDropdownOpen && constituencies.length > 0 && (
                        <div className="dropdown-menu" role="listbox">
                          <DropdownSearch 
                            query={constituencySearchQuery}
                            setQuery={setConstituencySearchQuery}
                            placeholder={t('HOME.SearchConstituencies')}
                          />
                          <div className="dropdown-list">
                            {filteredConstituencies.length > 0 ? (
                              filteredConstituencies.map(constituency => (
                                ...
                                >
                                  {constituency.constituancy_name} - {selectedDistrictName}
                                  <p className="state-name">{selectedStateName}</p>
                                </button>
                              ))
                            ) : (
                              <div className="no-results">{t('HOME.NoConstituenciesFound')}</div>
                            )}
                          </div>
                        </div>
                      )}
                  ...
                  <div className="form-group">
                    ...
                    <div className="dropdown-container">
                      ...
                      {isPollingBoothDropdownOpen && pollingBooths.length > 0 && (
                        <div className="dropdown-menu" role="listbox">
                          <DropdownSearch 
                            query={pollingBoothSearchQuery}
                            setQuery={setPollingBoothSearchQuery}
                            placeholder={t('HOME.SearchPollingBooths')}
                          />
                          <div className="dropdown-list">
                            {filteredPollingBooths.length > 0 ? (
                              filteredPollingBooths.map(booth => (
                                <button
                              ...
                              <div className="no-results">{t('HOME.NoPollingBoothsFound')}</div>
                            )}
                          </div>
                        </div>
                      )}
                  ...
                  <div className="btn-group">
                    ...
                    >
                      ...
                      {isLoading ? 'Searching...' : t('HOME.Search')}
                    </button>
                  </div>
                </div>
                ...
                {availablePDFs.length > 0 && (
                  <div className="pdfs-section">
                    <h2 className="pdfs-title">{t('HOME.AvailablePDFs')}</h2>
                      ...
                      <div className="pdf-table-scroll">
                        <table className="pdf-table">
                          <thead>
                            <tr>
                              <th>{t('HOME.SlNo')}</th>
                              <th>{t('HOME.PollingStationName')}</th>
                              <th>{t('HOME.Address')}</th>
                              <th>{t('HOME.ConstituencyName')}</th>
                              <th>{t('HOME.ImagePDF')}</th>
                              <th>{t('HOME.ConvertAction')}</th>
                            </tr>
                          </thead>
                          <tbody>
                                  ...
                                  {station.pdfs?.map((pdf, pdfIndex) => (
                                    <div key={`pdf-${pdfIndex}`} className="split-cell-row">
                                      <a
                                        ...
                                        onClick={(e) => {
                                          e.preventDefault();
                                          downloadImgPDF(pdf.attach_id, pdf.pdf_name, pdf.url);
                                        }}
                                      >
              ...
              <div className="space-y-6">
                <h2 className="main-title">
                  {t('HOME.UploadConvertToEditable')}
                </h2>
                ...
                {uploadedFiles.length > 0 && convertedFiles.length === 0 && (
                  <div className="space-y-6">
                    <h3 className="section-title">{t('HOME.UploadConvert')}<span className="title-rupee">{t('HOME.PerPage')}</span></h3>
                      ...
                      {uploadedFiles.map((file, index) => (
                        <div key={file.id} className="file-item">
                          <div className="file-info">
                            ...
                            <span className="file-details">{file.pages}{t('HOME.Pages')}</span>
                            <div
                    ...
                    <div className="pricing-section">
                      <div className="total-row">
                        <span className="total-label">{t('HOME.Total')}</span>
                      ...
                      <div className="btn-group">
                        ...
                        >
                          {isConverting ? 'Processing Payment...' : t('HOME.PayConvert')}
                        </button>
                      </div>
                    </div>
                  </div>
                ...
                {convertedFiles.length > 0 && (
                  <div className="space-y-6">
                    <div className="success-container">
                      ...
                      <h3 className="success-title">{t('HOME.ConversionCompleted')}</h3>
                      <p className="success-text">{t('HOME.ConversionCompletedMessage')}</p>
                    </div>
                    
                    <div className="space-y-3">
                      <h3 className="section-title">{t('HOME.ConvertedFiles')}</h3>
